...
class GoodsController extends Controller {
	 public function index(){
		...
		echo "window.location.href=\"/dist/index.html#/designer\"";
		echo "</script>";
	 }
	 ...
	 public function get_list()
	 {
		 ...
		 do{
			 ...
			 if($data["limit"])
			 {
				$rows = $db->where($where)->order(array('id'=>'desc'))->limit($data["begin"],$data["limit"])->select();
			 }else{
				$rows = $db->where($where)->order(array('id'=>'desc'))->select();
			 }
		 }while(0);
		 RSP($ret,$msg,array('list'=>$rows));
		 return 0;
	 ...
	 public function getRecentCustomize()
	 {
		...
		do{
			...
			if($id == false)
			{
				$rows = $db->where(array('is_online'=>1))->order(array('id'=>desc))->limit(1)->select();
			}else{
//				$rows = $db->where(array('is_online'=>1,"id"=>$id))->order(array('id'=>desc))->limit(1)->select();
				$rows = $db->where(array("id"=>$id))->order(array('id'=>desc))->limit(1)->select();
			}
		...
		$result = array('ret'=>$ret,'msg'=>$msg,'data'=>array('title'=>$title,'index_img'=>$index_img,
			'parts_arr'=>$parts_arr,'body_img_list'=>$body_img_list,'id'=>$id,'is_designer'=>$is_designer));
	 ...
	 public function get_customize_list()
	 {
		...
		do{
			...
			$rows = $db->field("id,title,img,visits,order_num")->where(array('is_designer'=>0,'id'=>array("EGT",33),'is_online'=>1))->order(array('id'=>desc))->limit("$start,$limit+1")->select();
		}while(0);
		if(count($rows) == $limit)
		{
			$is_more = 1;
	...
	public function get_designer_product_list()
	{
		...
		do{
			...
			$db = new \Home\Model\DressListModel();
			$db = D("dress_list");
			$data = $db->field("id,title,img,visits")->where(array('is_designer'=>1,'is_online'=>1))->order(array('id'=>desc))->limit($start,$limit)->select();
			if(count($data) >= $limit)
			{
				$is_more = 1;
				array_pop($data);
